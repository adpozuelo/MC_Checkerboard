...
def main():
    if len(sys.argv) < 3:
        print("Usage: python convert_to_lammps_atomic_style.py <input.atoms> <output.atoms_lammps> [sigma]")
        print("Default sigma is 12.631578947368421 (use 1.0 for reduced units)")
        sys.exit(1)
    ...
    for line in lines:
        ...
        if in_atoms:
            parts = stripped.split()
            # ellipsoid style Atoms format: atom_id type ellipsoid_flag density x y z
            atom_id = int(parts[0])
            itype = int(parts[1])
            x = float(parts[4])
            y = float(parts[5])
        ...
        if in_ellipsoids:
            parts = stripped.split()
            # ellipsoid style Ellipsoids format: atom_id shapex shapey shapez qw qi qj qk
            atom_id = int(parts[0])
            qw = float(parts[4])
            qi = float(parts[5])
            qj = float(parts[6])
    ...
    print(f"Writing atomic-style configuration with 4 atom types to '{output_path}'...")
    with open(output_path, 'w') as f:
        f.write("# LAMMPS atomic data file for site-site patchy model (5-site representation, centered)\n\n")
        ...
        for atom_id in sorted(atoms_dict.keys()):
            ...
            # Wrap the center coordinates back into the periodic box [-L/2, L/2]
            ...
            # Write center site: atom_id atom_type x y z
            f.write(f"{new_atom_id} {center_type} {cx:.8f} {cy:.8f} {cz:.8f}\n")
            new_atom_id += 1
            ...
            for u in tetra_vecs:
                ...
                # Write patch site: atom_id atom_type x y z
                f.write(f"{new_atom_id} {patch_type} {px:.8f} {py:.8f} {pz:.8f}\n")
                new_atom_id += 1
