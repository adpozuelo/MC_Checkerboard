...
def main():
    if len(sys.argv) < 3:
        print("Usage: python convert_to_lammps_atomic_reduced.py <input.atoms> <output.atoms_lammps>")
        print("Outputs coordinates directly in reduced/LJ units (compatible with mc_gpu.exe)")
        sys.exit(1)
    ...
    print(f"Writing atomic-style configuration (reduced/LJ units) to '{output_path}'...")
    with open(output_path, 'w') as f:
        f.write("# LAMMPS atomic data file for site-site patchy model (5-site representation, reduced/LJ units, centered)\n\n")
