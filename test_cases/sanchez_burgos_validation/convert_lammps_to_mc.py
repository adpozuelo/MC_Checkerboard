...
def main():
    lammps_data_path = "/home/e.lomba/sanchez_burgos_20260915/sanchez_burgos_lammps/validation_gpu/cube/final.data"
    output_dir = "/home/e.lomba/MC_Checkerboard/test_cases/sanchez_burgos_validation"
    ...
    # Reference patch vectors (unit vectors) matching generate.py exactly
    ...
    for mid in sorted(atoms_by_mol.keys()):
        ...
        assert len(core_list) == 1, f"Molecule {mid} does not have exactly 1 core: {core_list}"
        core = core_list[0]
        patches = patch_list
        ...
        assert len(patches) == n_patches, f"Expected {n_patches} patches, found {len(patches)}"
        
        # Relative patch vectors, minimum image with respect to core
        actual_dirs = []
        for p in patches:
            dr = p[2] - core_pos
        ...
        # Kabsch algorithm to find optimal rotation matrix R: actual = R @ ref
    ...
    # Write data.atoms for MC_Checkerboard
    data_atoms_path = os.path.join(output_dir, "data.atoms")
    ...
    with open(data_atoms_path, "w") as f:
        ...
        for i, m in enumerate(molecules):
            aid = i + 1
            atype = m['mc_type'] + 1 # 1-based for LAMMPS format
    ...
    print("data.atoms written successfully.")
    
    # Write datos.nml
    datos_path = os.path.join(output_dir, "datos.nml")
    print(f"Writing {datos_path}...")
    with open(datos_path, "w") as f:
        f.write("""&Control_Params
  istep_ini      = 0,
  istep_fin      = 0,
  Neq            = 0,
  Nmove          = 20,
...
-- sigma_LJ --
 ...
 1.000 1.000
""")
    print("datos.nml written successfully.")
